...
def initial_graph(edge_index, data_name: str = 'TEXAS'):
    ...
    try:
        with open("trained_model_state_" + data_name, "rb") as output_file:
            ...
            with open("trained_model_state_" + data_name, "wb") as output_file:
                pickle.dump(estimated_graph.state, output_file)


    #virtual_graph = estimated_graph.state['z']
    virtual_graph = estimated_graph.compute_mean_z(1000)

    virtual_graph = virtual_graph.to_dense()
    virtual_graph = (virtual_graph + torch.transpose(virtual_graph, 0, 1))
